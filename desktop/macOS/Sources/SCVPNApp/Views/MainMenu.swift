...
struct MainMenu: View {
    ...
    var body: some View {
        Group {
            ...
            // Пункт есть всегда: без него снявший компонент оказывался в
            // тупике — «Удалить» исчезало, а поставить обратно было нечем,
            // кроме побочного пути через попытку подключения.
            Button(model.helperInstalled
                   ? "Переустановить системный компонент…"
                   : "Установить системный компонент…") { model.installHelper() }
        ...
        // Состояние компонентов считается по файлам и по статусу службы, а их
        // меняет не только это приложение. Пересчитываем в момент открытия
        // меню: иначе пункты показывали бы то, что было на запуске.
        .onAppear { model.refreshComponents() }
    }
