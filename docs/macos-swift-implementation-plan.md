...
> **Статус Фазы 0 на 2026-08-15: закрыты 0.1, 0.3, 0.4, 0.7, 0.8. Открыты
> 0.2, 0.5, 0.6, 0.9.**
>
...
>
> Три пункта переспрашиваются не потому, что их не прогнали, а потому что
> прогон дал негодные данные: 0.2 — слишком короткое ожидание после
> `kickstart`; 0.5 и 0.6 — BTM помнил согласие, выданное прежнему зонду, и
> отвечал памятью вместо папки. Зонд исправлен (`--suffix`, абсолютный путь в
> журнале). Подробности — под каждой задачей.
>
> **Фазу 3 закрывать нельзя, пока не закрыт 0.6:** от него зависит, нужна ли
...
**Первый заход не дал ответа, требуется переспрос.** После подмены бандла и
...
**Замер испорчен, требуется переспрос.** BTM запоминает согласие по
...
**Замер неоднозначен, требуется переспрос.** Из `~/Downloads` статус дошёл до
