...
Одно поле на все случаи — тип определяется по самому тексту, а не выбором
пользователя: ссылку сервера и конфиг WireGuard парсер узнаёт, всё остальное с
http(s) считается подпиской. Спрашивать «это ссылка или подписка?» значило бы
перекладывать на человека то, что программа выясняет сама. Поле многострочное
как раз поэтому: конфиг WireGuard в одну строку не помещается.
"""
...
class AddDialog(QDialog):
    ...
    def __init__(self, parent=None) -> None:
        ...
        caption = QLabel(
            ...
            "wireguard:// / awg://), конфиг WireGuard целиком (блок [Interface])\n"
