...
> **Статус Фазы 0 на 2026-08-15: закрыта полностью.**
>
...
>
> **Задача 0.9 закрыта** — см. её раздел. Раздел 2.4 спецификации оказался
> неполон: сверх пятнадцати исходных инвариантов набралось семь (№16–22) плюс
> семь свойств, дописанных по итогам сплошного чтения.
>
...
**Выполнена 2026-08-15, после Фазы 6.** 90 проверок `test_native.py` прочитаны
и сопоставлены с перенесёнными. Свойства, которые держались только там и в
Swift оказались непокрыты, — дописаны (`TunGuaranteesTests`):

| Свойство | Почему важно | Проверка |
|---|---|---|
| Команда `stop` снимает туннель **сама**, без обрыва | Две независимые гарантии обязаны работать поодиночке, иначе «починка» одной тихо ломает другую | `test_stop_command_alone_drops_the_tunnel` |
| Обрыв соединения снимает туннель **сам**, без команды | То же с другой стороны | `test_closing_the_connection_alone_drops_the_tunnel` |
| Закрытие будит читателя, стоящего в чтении | Единственная страховка от мутации «убрали shutdown»; без него round-trip-проверки виснут насмерть | `test_closing_wakes_a_reader_blocked_in_stream` |
| `tun_stack` доезжает до JSON на диске | Значение может потеряться по дороге от настроек через демона | `test_tun_stack_setting_reaches_the_singbox_config` |
| Умолчание `gvisor` приходит из **настоящих** настроек | Предыдущая проверка передаёт значение руками и не заметит, если ключ вычеркнуть из умолчаний | `test_default_stack_is_gvisor_and_comes_from_real_settings` |
| Крах обслуживания на **чужом** соединении не трогает туннель | Обслуживание валится не только на чтении | `test_a_broken_foreign_connection_leaves_the_tunnel_alone` |
| Крах на **своём** соединении туннель снимает | Пара к предыдущей: без неё легко починить одну сторону, сломав другую | `test_a_broken_owning_connection_drops_the_tunnel` |

**Вывод по разделу 2.4 спецификации: он был неполон.** Задача поставлена
верно. Всего сверх пятнадцати исходных инвариантов набралось семь (№16–22 в
Приложении А) плюс эти семь свойств — и два из них, SIGPIPE и `deinit` сокета,
держались вообще не на коде, а на устройстве Python.
...
- [x] **Шаги:** реализовать, проверить оба правила руками, коммит.

**Результат.** Меню собрано, 288 проверок зелёные. Оба правила закрыты:
«Удалить компоненты TUN…» показывается по `CoreDownloader.tunPresent()`, а
префлайт TUN идёт «сначала права, потом компоненты» — `AppModel.tunPreflight`.

**Само меню — SwiftUI и проверке не поддаётся**, поэтому проверками закрыты
функции под ним: что `tunPresent` и `privileged` остались независимыми
вопросами (их слияние и давало неснимаемый круг), что у каждой настройки меню
есть умолчание, и что каждый пункт разбирается кодом — пункт, которого разбор
не понимает, молча свалился бы в умолчание, и переключатель показывал бы одно,
а работало другое.

Заведён третий вопрос сверх плана: `helper_version` **не** должен лежать в
`defaultSettings`. Появись он там, первая же сверка решила бы, что демон уже
нужной сборки, и перерегистрация из Задачи 3.2 не случилась бы никогда.

Окно загрузки (`ProgressSheet`) сделано здесь, а не в Задаче 6.5: пункты меню
«Скачать…» без него ничего бы не показывали. Оба правила плана соблюдены —
отмены нет, полоска для sing-box остаётся бегущей.
...
- [x] **Шаги:** реализовать по одному диалогу, ручная проверка, коммиты.

**Результат.** Четыре диалога собраны: добавление, раздельное туннелирование,
подписки, сканер QR. Окно загрузки сделано раньше, в Задаче 6.4.

**Замер веса бандла — главное число всего плана.** Собранный Swift-бандл
весит **2,6 МБ** против **223 МБ** у Python-версии: в 86 раз меньше. Задача
−1.3 взвешивала переписывание по двум выигрышам, вес был первым из них.

Ушли `opencv-python-headless` (118 МБ по замеру Задачи −1.2), `qrcode` и весь
PySide6. Сканер — `AVCaptureMetadataOutput`, он распознаёт QR сам: ни ручного
цикла захвата кадров, ни детектора. Генерация — `CIFilter.qrCodeGenerator`.

**Что закрыто проверками, а не разметкой.** Сами диалоги — SwiftUI, но правила
под ними проверяемы: тип вставленной строки определяется по ней самой; «авто»
сохраняется как `route_mode`, а не `split_mode` (иначе туннель поднялся бы не
тем, чем просили); список приложений переживает переключение на «всё через
VPN»; и главное — имя, которое диалог сохранит после нормализации, **проходит
проверку демона**, а не отваливается уже при подключении.
...
> сборку переведены на Swift-версию.
>
> **Планка macOS 13.0 подтверждена замером, а не намерением:**
> `otool -l` на собранном бинарнике даёт `minos 13.0`, сборка с явным
> `-target arm64-apple-macosx13.0` проходит без единого предупреждения о
> доступности. Заменять ничего не пришлось — ни один использованный API не
> новее 13.0. Самое близкое к границе — сам `SMAppService`, он ровно 13.0.
...
- [x] **Шаги:** переписать, вычитать, коммит.

### Задача 7.2: Проверка версии macOS

- [x] **Шаг 1:** Собрать под `--target arm64-apple-macosx13.0`, убедиться, что
  ни один используемый API не новее 13.0.
- [x] **Шаг 2:** Если что-то новее — заменить, а не поднимать планку.
- [x] **Шаг 3:** Коммит.
...
> **Пройдена 2026-08-16.** macOS 26.6.1, Apple Silicon, бандл из
> `/Applications`, компонент через `SMAppService`.
>
> | Шаг | Результат |
> |---|---|
> | 1. Режим «прокси» | `Enabled: Yes`, `Server: 127.0.0.1`, `Port: 10809`; после отключения `Enabled: No` |
> | 2. Переключение на TUN в том же окне | без перезапуска приложения |
> | 3. Туннель поднят | `sing-box` PID 34674, выход `144.31.232.142` — сервер |
> | 4. **Dead-man's switch** | демон **34503 до и после** `kill -9` приложения, `sing-box` 1 → 0, маршрут вернулся на `en0`, адрес домашний |
> | 5. Раздельное туннелирование | **работает** |
>
> **Шаг 4 — главное.** PID демона не сменился, значит туннель снял именно
> обработчик обрыва соединения, а не подметание сироты новым демоном, поднятым
> launchd. Это и есть свойство, ради которого написана вся архитектура.
>
> **Про шаг 1.** Проверять системный прокси через `curl` бессмысленно:
> консольные утилиты читают переменные окружения и про настройку
> `networksetup` не знают. Это записано и в README, но в первую редакцию
> инструкции проверки не попало. Смотреть надо браузером.
>
> **Про шаг 5 и Safari.** Первая попытка дала одинаковые адреса и выглядела
> как «правила не работают». Причина не в правилах: **Safari сам в сеть не
> ходит**, за него это делает отдельный XPC-процесс
> `com.apple.WebKit.Networking`. Правило `process_name: ["Safari"]` не может
> совпасть ни с одним запросом.
>
> Решающая проверка на `curl`, который делает сеть сам: `curl` (исключён) вышел
> через `144.31.85.228`, `python3` (в туннеле) — через `144.31.232.142`. Два
> процесса в один момент на одной машине ушли **разными путями**, и различает
> их ровно наше правило.
>
> Первый адрес оказался чужим сервером, а не домашним, потому что на машине
> параллельно работает `ChatVPN`: вышедший из нашего туннеля `curl` попал в
> чужой. На вывод это не влияет — важно, что пути разошлись.
>
> Ловушка с Safari описана в подсказке диалога и в README: выбрать его в списке
> можно, а работать это не будет, и человек решит, что сломан раздельный
> туннель.
...
- [x] Шаг 1: режим «прокси», проверка `networksetup -getwebproxy Wi-Fi`.
- [x] Шаг 2: переключение на TUN в том же окне, первое включение просит
  разрешение в System Settings.
- [x] Шаг 3: фиксация, что туннель действительно поднят (непустой `SB_PID`, а
  не наличие `utun` — на типичной машине уже стоят посторонние `utun0..utun9`).
- [x] Шаг 4: dead-man's switch — `kill -9` приложения, PID демона обязан
...
- [x] Шаг 5: раздельное туннелирование по приложениям — **шаг, который ни разу
  не выполнялся** (README это честно признаёт).

### Задача 8.2: Исход шага 5

- [x] **Если правила `process_name` работают:** снять ⚠️-оговорку в README.
  **Сработали, оговорка снята.**
- [x] **Если не работают:** честный исход не «оставить как есть», а:
...
**Сделано 2026-08-15**, не дожидаясь остального Шага 8.3: от исхода ручной
проверки карта не зависит, а налог начинается с первой же правки в
`desktop/shared/`, то есть уже сейчас. В файл вошли и список того, чего в
Swift-версии нет намеренно, и перечень форматов, которые обязаны совпадать
побайтно.
- [x] **Шаг 4:** Коммит.
