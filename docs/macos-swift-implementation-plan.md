...
> **Статус Фазы 0 на 2026-08-15: НЕ ЗАКРЫТА, Задачи 0.1–0.6 и 0.9 не выполнены.**
>
> Задачи 0.1–0.6 требуют перезагрузки Mac, `sudo`, ввода пароля администратора
> и нажатия «Разрешить» в System Settings → Login Items. Выполнить их из
> неинтерактивной сессии нельзя, а рисовать результат, которого не было, —
> хуже, чем оставить пункт открытым: на эти четыре ответа опираются Задачи 3.2,
> 3.3 и стоп-условие всего плана A (`KeepAlive`, Задача 0.4).
>
> **Что это значит для порядка работ.** Фазы 1, 2, 4 и 5 от исходов Фазы 0 не
> зависят: демон, ядро логики и платформенный слой пишутся одинаково при любом
> ответе. Зависит только Фаза 3 (установщик) — её нельзя закрывать, пока
> Задачи 0.1–0.6 не прогнаны живьём. Поэтому порядок такой: Фаза 1 → Фаза 2 →
> Фаза 0 руками → Фаза 3.
>
...
**Если нет:** в Задаче 3.2 появляется безусловный `unregister()` + `register()`
при несовпадении версии демона, а `ExitTimeOut` проверяется отдельной
XCTest-проверкой на содержимое plist в бандле.
...
- [ ] **Шаг 3:** PID обязан смениться на новый. Записать результат.
...
- [ ] **Шаг 3: Прогнать ручную проверку dead-man's switch (шаги 1–4 из README)**

С поправкой: `pgrep -f "run.py --helper"` заменяется на
`pgrep -f scvpn-helper`.

- [ ] **Шаг 4: Записать результат в этот файл**
...
> **Статус: Шаги 1 и 5 сделаны, Шаги 2–4 не выполнены.**
>
> Скрипт `Tools/install-helper-dev.sh` написан, правка `installed()` из раздела
> 0.3 внесена и помечена комментарием. Сам прогон требует `sudo launchctl
> bootstrap`, ввода пароля администратора и живого VPN-подключения с потерей
> сети на время проверки — из неинтерактивной сессии этого не сделать.
>
> **Фаза 2 закрытой не считается.** Закрывает её именно этот шаг: 117
> XCTest-проверок держат каждое свойство по отдельности, но того, что
> Python-приложение говорит со Swift-демоном по живому сокету и dead-man's
> switch срабатывает на настоящем `sing-box`, не проверял никто.
...
- [ ] **Шаг 2–5:** запустить, реализовать, зелёные, коммит.
...
- [ ] **Шаг 1:** Добавить `waitUntilReady(timeout:)` — опрос `svc.status` раз в
  секунду, пока не `.enabled` или не истечёт таймаут.
- [ ] **Шаг 2:** Если Фаза 0 (Задачи 0.2, 0.3) показала, что подмена бандла не
  подхватывается — добавить `helper_version` в `settings.json` и безусловный
  `unregister()` + `register()` при несовпадении. Если показала обратное — этот
  шаг вычёркивается, и здесь записывается, почему.
- [ ] **Шаг 3:** Коммит.

### Задача 3.3: Проверка расположения бандла

**Файлы:**
- Modify: `Sources/SCVPNApp/HelperInstaller.swift`

Выполняется только если Фаза 0 (Задачи 0.5, 0.6) подтвердит ограничение.

- [ ] **Шаг 1:** Проверка `Bundle.main.bundleURL` на нахождение вне
  `~/Documents`, `~/Desktop`, `~/Downloads` — с тем же текстом, что в
  Задаче −1.1.
- [ ] **Шаг 2:** Коммит.
...
- [ ] **Шаг 1:** При первом запуске Swift-версии проверить существование этого
  файла.
- [ ] **Шаг 2:** Если он есть — показать диалог «Найден старый системный
...
- [ ] **Шаг 1:** Написать спайк на `URLSession` и проверить живьём против
  поднятого `xray`.
- [ ] **Шаг 2:** Если не работает — использовать `Process` с
  `/usr/bin/curl -s -m 6 -x http://127.0.0.1:PORT https://api.ipify.org`.
  `curl` есть в macOS всегда. Записать выбор здесь.
- [ ] **Шаг 3:** Проверки на чистую часть:
...
- [ ] **Шаг 4–5:** зелёные, коммит.

### Задача 4.8: Приёмка Фазы 4

- [ ] **Шаг 1:** Прогнать парсинг на том же наборе ссылок, что и
  `smoke_test.py`.
- [ ] **Шаг 2:** Прочитать существующий `profiles.json` пользователя, записать
...
- [ ] **Шаг 1: Падающие проверки** (против `Stand` из Задачи 2.7)
...
- [ ] **Шаг 2–5:** реализовать, зелёные, коммит.
...
- [ ] **Шаги:** каркас, привязка к `Tun`/`XrayRunner`, ручная проверка, коммит.
...
- [ ] **Шаги:** реализовать, ручная проверка на масштабе 1x и 2x, коммит.
...
В Python его не было, потому что там его давал сам язык: SIGPIPE заглушён по
