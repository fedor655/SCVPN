...
> **Поддержка прекращена.** Версия дописана и работает, кроме самого туннеля:
> он требует entitlement `com.apple.developer.networking.networkextension`,
> который Apple выдаёт только за $99 в год. Платить и продлевать подписку ради
> одной платформы мы не готовы, поэтому версия не развивается и не
> поддерживается. Код оставлен рабочим — если аккаунт появится, продолжать
> нужно с Фазы 0 плана.
